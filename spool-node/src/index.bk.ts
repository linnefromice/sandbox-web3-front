...
const main = async () => {
  ...
  const balances = await contract.callStatic
    .getSmartVaultAssetBalances(vaultSrETH, false)
    .catch(e => {
      console.log(
        `Fail to getSmartVaultAssetBalances with doFlush=false: ${vaultSrETH}`
      );
      console.error(e);
      return [];
    });
  ...
  const balances2 = await contract.callStatic
    .getSmartVaultAssetBalances(vaultSrETH, true)
    .catch(e => {
      console.log(
        `Fail to getSmartVaultAssetBalances with doFlush=true: ${vaultSrETH}`
      );
      console.error(e);
      return [];
    });
